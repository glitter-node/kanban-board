<div class="space-y-3">
    ...
    <template x-if="!selectedCardCommentsLoading && selectedCardCommentsError">
        <x-ui.state.error title="Comments unavailable" retryLabel="Retry" @click="loadComments(selectedCard.id)">
    ...
    <template x-for="comment in selectedCardComments" :key="comment.id">
        <x-ui.card class="px-4 py-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-medium" x-text="comment.author_name || currentUserName"></p>
                    <p class="ui-meta" x-text="comment.created_at ? new Date(comment.created_at).toLocaleString() : ''"></p>
                </div>
            </div>

            <p class="mt-3 whitespace-pre-wrap text-sm ui-meta" x-text="comment.body"></p>
        </x-ui.card>
    </template>
</div>
