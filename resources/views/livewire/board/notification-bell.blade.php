<div
    x-data="notificationBell({ userId: @js($userId), initialNotifications: @js($initialNotifications) })"
    x-init="init()"
    class="relative"
>
        ...
        <x-slot name="trigger">
            ...
            >
                ...
                <span
                    ...
                    x-cloak
                    class="absolute -right-1 -top-1 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full ui-badge ui-badge-sm ui-badge-error px-1 text-[10px] font-semibold"
                    x-text="unreadCount"
                ></span>
            </x-ui.button>
        </x-slot>
    ...
    <div x-show="open" x-cloak @click.outside="open = false" class="absolute right-0 z-50 mt-3 w-96 overflow-hidden">
        <x-ui.panel padding="none" class="rounded-3xl">
            ...
            <div class="max-h-[24rem] overflow-y-auto">
                <template x-if="notifications.length === 0">
                    <x-ui.surface class="rounded-none border-x-0 border-b-0 border-t-0 px-5 py-10 text-center shadow-none">
                        <p class="text-sm ui-meta">No notifications yet.</p>
                    </x-ui.surface>
                </template>
                ...
                <template x-for="notification in notifications" :key="notification.id">
                    <x-ui.surface
                        as="article"
                        class="rounded-none border-x-0 border-b-0 px-5 py-4 shadow-none"
                        :class="notification.read_at ? '' : 'ui-surface-elevated'"
                    >
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-medium" x-text="notification.payload?.title || notification.type"></p>
