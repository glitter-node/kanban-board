<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('boards.create') }}" class="btn-primary focus:border-border">
                + New Board
            </a>
        </div>
    </x-slot>
            ...
            @if ($boards->isEmpty())
                <div class="ui-panel overflow-hidden sm:rounded-lg">
                    <div class="p-12 text-center">
                        ...
                        <div class="mt-6">
                            <a href="{{ route('boards.create') }}" class="btn-primary">
                                + Create New Board
                            </a>
                        </div>
                    </div>
                            ...
                            <a href="{{ route('boards.show', $board) }}" class="block p-6">
                                <div class="flex items-start justify-between">
                                    ...
                                    <div class="ml-2 flex items-center gap-1" x-data="{ open: false }">
                                        <button @click.prevent="open = !open" aria-label="Open board actions" class="btn-icon p-1 bg-primary text-white">
                                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                            </svg>
                                        </button>

                                        <div x-show="open" @click.away="open = false" x-cloak class="ui-panel absolute right-0 z-10 mt-20 w-36 p-1">
                                            ...
                                            <form method="POST" action="{{ route('boards.destroy', $board) }}" @click.stop onsubmit="return confirm('Are you sure you want to delete this board?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="block w-full rounded-md px-4 py-2 text-sm transition hover:bg-primary hover:text-white">
                                                    Delete
                                                </button>
                                            </form>
                                        </div>
